...
class LP_Addon_Collections {
	...
	function __construct() {
		...
		add_filter( 'rwmb_meta_boxes', array( $this, 'collection_settings_metabox' ), 20 );
		add_filter( 'rwmb_meta_boxes', array( $this, 'course_collection' ), 20 );
	...
	function register_collections() {
		$labels = array(
			'name'               => _x( 'Programs', 'Post Type General Name', 'learnpress-collections' ),
			'singular_name'      => _x( 'Program', 'Post Type Singular Name', 'learnpress-collections' ),
			'menu_name'          => __( 'Programs', 'learnpress-collections' ),
			'parent_item_colon'  => __( 'Parent Item:', 'learnpress-collections' ),
			'all_items'          => __( 'Programs', 'learnpress-collections' ),
			'view_item'          => __( 'View Program', 'learnpress-collections' ),
			'add_new_item'       => __( 'Add New Program', 'learnpress-collections' ),
			'add_new'            => __( 'Add New', 'learnpress-collections' ),
			'edit_item'          => __( 'Edit Program', 'learnpress-collections' ),
			'update_item'        => __( 'Update Program', 'learnpress-collections' ),
			'search_items'       => __( 'Search Programs', 'learnpress-collections' ),
		...
		$args = array(
			...
			'hierarchical'       => true,
			'rewrite'            => array( 'slug' => 'programs', 'hierarchical' => true, 'with_front' => false )
		);
		register_post_type( 'lp_collection', $args );
		///flush_rewrite_rules();
		add_rewrite_tag( '%collection_page%', '([^&]+)' );
		add_rewrite_rule( '^programs/([^/]*)/page/(.*)', 'index.php?lp_collection=$matches[1]&collection_page=$matches[2]', 'top' );
	}
	...
	*/
	function admin_tabs_info( $arr ) {
		$arr[11] = array(
			...
			"name" => __( "Programs", "learnpress-collections" ),
	...
	*/
	 ...
	 * @return mixed
	 */
	function collection_settings_metabox( $meta_boxes ) {
		$prefix       = '_lp_';
		$meta_boxes[] = array(
			...
			'fields'   => array(
				...
				array(
					'name' => __( 'Program Description', 'learnpress-collections' ),
					'id'   => "program-description",
					'type' => 'textarea'
				)
			)
		);
		return apply_filters( 'learn_press_collection_settings_metabox', $meta_boxes );
	}
	 ...
	 * @return mixed
	 */
	function course_collection( $meta_boxes ) {
		$prefix       = '_lp_';
		$meta_boxes[] = array(
		...
		return apply_filters( 'learn_press_course_collection_metabox', $meta_boxes );
	}
